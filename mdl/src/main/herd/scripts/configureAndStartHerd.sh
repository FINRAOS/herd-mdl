...
function addStackTagsToCloudFront(){
    ...
    # execute_cmd "cloudfront_tags=$( echo jq -r '.[]' | jq '{Items: .}' <<<\"${stack_tags}\" )"
    # execute_cmd "aws cloudfront tag-resource --resource arn:aws:cloudfront::328703175456:distribution/EJ0W4JB9Y2Q24 --tags \"$cloudfront_tags\""
}
